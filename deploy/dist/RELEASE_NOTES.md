...
- **v1.0.10**：OTA 升级新增 **CRC32/IEEE 校验**（与既有 SHA-256 并存，固件上传时同步计算并随 `ota.push` 下发，设备可先做 CRC32 快速校验再做 SHA-256 完整性校验，旧固件 `crc32=0` 仅校验 SHA-256）；**OTA 下发严格按设备协议选路**（TCP/DTU 设备走网关、MQTT 设备走 EMQX，去掉无脑 MQTT 回退，设备离线如实计入未送达）；系统管理页 **Vben 风格重构**（统一页头 + 设计令牌，六个页面全部套用，更专业美观）；页面切换动画改为交叉淡入（不弹出、不白屏）。
- **v1.0.9**：新增**平台超管后台**（admin 角色专属）——系统运行状态（版本/各组件健康/全局统计）、**热更新参数**（注册开关/令牌有效期/日志保留天数，改完立即生效）、平台公告管理（markdown + 发布/下线 + 控制台铃铛与概览页展示）、帮助中心在线编辑、全量用户管理（跨账号/角色/状态）、全局日志检索（设备日志 + 消息轨迹）；新增**对象存储**支持（local 本地磁盘 / s3 兼容：阿里 OSS/腾讯 COS/MinIO，公开读桶 + 短随机对象名适配 4G 模组，**对象存储配置可在超管后台在线修改并热重载**）；OTA 固件上传改走 storage 抽象 + SHA-256 校验；参数配置页中文化。
- **v1.0.8**：修复 TCP 透传设备上下线状态不更新（网关/模拟器状态键 clientid 改用与 MQTT 一致的纯拼接格式，`ParseClientID` 按字符数解析不再误带前导点）；Modbus 单次 RTU 请求超时改为**产品级可配置**（`requestTimeout`，默认 3 秒、钳制 3-30 秒，控制台 Modbus 产品表单可直接设置，适配响应慢的从机）；ClientID 明确为 `{productId}{deviceName}` **纯拼接无分隔符**（点号仅作设备名字符，TCP 注册包三元组与 MQTT 对齐）；前端新增**公开官网首页**（免登录）与控制台 `/console/*` 路由迁移（旧路径自动重定向）。
