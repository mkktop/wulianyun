...
- **v1.0.7**：**第二批 + 第三批安全/可靠性审计修复**——JWT 中间件回查 DB（禁用/降级即时生效）；WS 改首帧认证（JWT 不再进 URL，防泄露进访问日志）+ 指数退避重连；规则引擎幂等（三态求值防字段缺失误清告警、firing 状态缓存 + 原子去重，离线告警不再重复插行、上线自动恢复）；遥测批量化失败重试不丢数据；优雅关闭顺序修正 + 等待最后一次 flush；valid=false 遥测不污染 latest/shadow/规则；系统方法键无条件剔除；HTTP 上报补禁用检查；轨迹/日志批量化（去每包 INSERT）；WS 心跳防半开泄漏；Modbus 超时帧不再串设备；影子 retained 持锁发布 + 多实例一致性 + 深拷贝防 data race；Kafka 未实现规则拒绝 + MQTT 桥接锁外 Connect 防拖慢 + 重连防泄漏；round2 负数对称舍入；点位/采集组变更即时生效；codec 长度上限 + 真编译校验 + recover。文档全面校正与代码一致性（产品ID 仅固定格式不兼容旧版、WS 首帧认证、令牌绑定、JWT 密钥告警、firing Redis 键等）。
- **v1.0.6**：**14 项安全审计修复**——MQTT 调试台仅平台超管可用并限制 `$SYS`/通配主题（原任意登录用户可窃取全平台遥测/跨租户下发）；设备模拟器归属校验（原可枚举设备 ID 跨租户冒认、注入遥测）；固件上传扩展名白名单 + `/uploads` 强制附件下载（原可上传 HTML 存储型 XSS）；TCP 注册包有界读取（原无长度上限可内存 DoS）；EMQX 动态 Token 绑定 clientid（原可跨设备冒认）；OpenAPI 签名覆盖 Method+Path+BodyHash（原仅 AppKey+Timestamp 可篡改重放）及 nginx 补 `/openapi/` 代理；另修复 WS 广播并发崩溃、状态队列阻塞停摆、跨实例下行黑洞、Redis 抖动停采、点位保存非事务、32 位寄存器写入截断、广播未编码、Webhook SSRF。功能：产品 ID 统一为固定 12 字符（2 字母+10 数字）、ClientID 按字符数解析（设备名可含点号）；属性设置面板展示各可写属性期望状态、运行日志显示上下行详情、历史曲线单图切换 + 时间范围查询 + CSV 导出；设备状态事件 FIFO 串行 + 时间戳守卫防乱序、影子期望值 retained 必达。
